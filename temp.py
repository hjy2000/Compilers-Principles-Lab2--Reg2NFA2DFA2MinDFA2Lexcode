# -*- coding: utf-8 -*-
"""
Created on Thu Jan  6 04:25:52 2022
...
#生成词法分析程序
#import copy
stringCode ="#include<stdio.h>\nint count=0;\nint main()\n{\nif(ch >= 'a' && ch <='z')\n{"

def genAnd(chList,chList1):
    global stringCode
    global dfaStatusChart
    global wordsChart
    
    for i in range(len(chList)):
        for j in range(len(chList[i])):
            ch=chList[i][j]
            stringCode+="\nif(ch=='"+ch+"')\n{count+=1;\n}\n"
            
def genSingle(ch,ch1):
    global stringCode
    global dfaStatusChart
    global wordsChart
    stringCode+="\nif(ch=='"+ch+"')\n{count+=1;\n}\n"
    ...
    for i in range(len(chartStatus)):
        ...
        for j in range(2,2+len(wordsChart)):
            if dfaStatusChart.iloc[i+1,j]>='A' and dfaStatusChart.iloc[i+1,j]<='Z':
                tempList.append(dfaStatusChart.iloc[0,j])
                tempList1.append(dfaStatusChart.iloc[i+1,j])
                count+=1
        print(tempList)
        if count>1:
            genAnd(tempList,tempList1)
        elif tempList!=[]:
            genSingle(tempList[0],tempList1[0])
        else:
            pass
        
        orList.append(tempList)
        orList.append(tempList1)
        
    print(orList)
    stringCode+="\nif(count==str.size())\n{\ncout<<\"error\";\n}\nreturn 0;\n}"
    
def show_nfaChart(re):
    run(re)
...
if __name__ == '__main__':
    ...
    window.resizable(False,False)#不允许改变窗体尺寸
    e = StringVar()
    ...
    #lambda :fun(x)
    B1=tk.Button(window,text='表达式转NFA',height = 2,width = 10,command=lambda:show_nfaChart(re))
    ...
    B5=tk.Button(window,text="更新Entry值",height = 2,width = 10,command=renewEntry)
    B5.grid(row=1,column=7)
    
    B6=tk.Button(window,text="关于",height = 2,width = 10,command=show_Info)
